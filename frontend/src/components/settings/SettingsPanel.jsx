...
function Account({ userProfile }) {
  ...
  const [fullName, setFullName] = useState(userProfile?.full_name || 'SatQuery User');
  const [username, setUsername] = useState(userProfile?.username || '@satquery_user');
  const [isSaving, setIsSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  if (deleted) return <div className="settings-success"><Check size={18} /><div><strong>Account deleted</strong><p>Your account data has been permanently deleted. Please refresh the page.</p></div></div>;
  const email = userProfile?.email || 'user@example.com';

  const saveProfile = async () => {
    setIsSaving(true);
    try {
      await api.request('PUT', '/api/users/me', { full_name: fullName, username: username });
      setSaved(true);
  ...
  const deleteAccount = async () => {
    if (!window.confirm('Permanently delete your account and all data? This action CANNOT be undone.')) return;
    try {
      await api.request('DELETE', '/api/users/me');
      localStorage.clear();
      setDeleted(true);
  ...
  return <section className="settings-section">
    <div className="account-card" style={{ flexDirection: 'column' }}>
      <div style={{ display: 'flex', gap: '15px' }}>
        <div className="account-avatar"><UserRound size={20} /></div>
        <div className="account-details">
          <div><span>Name</span><input type="text" value={fullName} onChange={e => setFullName(e.target.value)} style={{ textAlign: 'right', border: 'none', background: 'transparent', outline: 'none', fontWeight: 500, color: 'var(--text)', fontSize: '12px', width: '200px', padding: 0 }} /></div>
          <div><span>Username</span><input type="text" value={username} onChange={e => setUsername(e.target.value)} style={{ textAlign: 'right', border: 'none', background: 'transparent', outline: 'none', fontWeight: 500, color: 'var(--text)', fontSize: '12px', width: '200px', padding: 0 }} /></div>
          <div><span>Email</span><strong>{email}</strong></div>
        </div>
      </div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px', alignItems: 'center', gap: '10px' }}>
        {saved && <span style={{ color: 'var(--accent)', fontSize: '12px' }}>Saved!</span>}
        <button onClick={saveProfile} disabled={isSaving} style={{ background: 'var(--accent)', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>{isSaving ? 'Saving...' : 'Save Profile'}</button>
      </div>
    </div>
    <div className="danger-card">
      <div><strong>Delete account</strong><p>Permanently remove your account and associated data.</p></div>
      <button className="danger-btn" onClick={deleteAccount}><Trash2 size={14} /> Delete</button>
    </div>
  </section>;
}
...
export default function SettingsPanel({ theme, setTheme, accent, setAccent, contrast, setContrast, userEmail, conversations = [], onClose }) {
  ...
  const pages = {
    ...
    account: <Account userProfile={userProfile || { email: userEmail }} />,
    storage: <Storage conversations={conversations} />,
    usage: <Usage conversations={conversations} />,
