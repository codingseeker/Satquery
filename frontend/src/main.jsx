...
function App() {
  ...
  // Also load the user's previous conversations so the sidebar is populated.
  React.useEffect(() => {
    ...
    fetch(`${apiBase}/api/conversations`, {
      ...
      .then(async res => {
        if (res.status === 401) {
          ...
          // Transform backend chat format into frontend format
          const formatted = data.map(c => ({
            id: c.id,
            backendId: parseInt(c.id, 10),
  ...
  async function selectChat(id) {
    ...
    if (conv && conv.messages.length === 0 && conv.backendId) {
      try {
        const { getConversation, sendQuery } = await import('./services/chatService');
        // Hack: adaptAnalysisResponse isn't exported, but it's identical to the result of sendQuery.
        // We'll just manually map it inline.
        const data = await getConversation(conv.backendId);
        if (data && data.messages) {
          const formattedMessages = data.messages.map(msg => {
             let adaptedAnalysis = null;
             if (msg.analysis) {
                 const raw = msg.analysis;
                 adaptedAnalysis = {
                    answer: raw.answer || '',
                    task: raw.task || 'vqa',
                    taskLabel: raw.task_label || raw.taskLabel || 'Analysis',
  ...
  async function regenerate(msgId) {
    ...
    if (aiIdx < 0) return;
    const prevMsgs = [...msgs].slice(0, aiIdx).reverse();
    const userMsg = prevMsgs.find(m => m.role === 'user');
    if (!userMsg) return;
